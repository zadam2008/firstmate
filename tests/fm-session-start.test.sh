...
EOF
  ...
  # gh-axi, not node: the constrained BASE_PATH system dirs may genuinely hold
  # a node (e.g. a distro package at /usr/bin/node), which would mask the
  # removal; an npm-global axi tool never lives there.
  rm -f "$fakebin/gh-axi"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # gh-axi, not node: BASE_PATH's system dirs may hold a real node (e.g.
  # /usr/bin/node), which would mask the removal; gh-axi never lives there.
  rm -f "$fakebin/gh-axi"
